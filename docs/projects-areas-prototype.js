...
function setBoardExpanded(expanded) {
  ...
  const button = document.getElementById('expandBoard');
  button.setAttribute('aria-expanded', String(expanded));
  button.textContent = expanded ? '← Focus in progress' : 'Expand board →';
  updatesOpen = !expanded;
  grid.querySelector('.pp-home-updates').open = updatesOpen;
}
...
function moveMission(id, status) {
  ...
  if (status === 'done' && ['executing', 'verification'].includes(mission.phase) && mission.conditions.every(condition => condition.done) && tasks.every(task => task.status === 'done')) {
...
function changeTaskStatus(id, status) {
  ...
  if (task.action?.startsWith('approve') && status === 'done' && task.status !== 'done') {
    app.navigate({ kind: 'mission', id: task.missionId });
    app.notice('Review and approve this item in the mission view.');
    return;
  }
...
document.addEventListener('click', event => {
  ...
  if (button.dataset.view) app.navigate({ kind: button.dataset.view, ...(button.dataset.id ? { id: button.dataset.id } : {}) });
    ...
    app.state = createPrototypeState(); workFilter = 'mine';
