defmodule CascadeWeb.ContentController do
  ...
  def create_vault(conn) do
    authenticated(conn, [user_only: true, error: "Could not create vault"], fn conn, auth ->
  ...
  def create_folder(conn, vault_id) do
    content(conn, {:vault, vault_id}, [write: true, error: "Could not create folder"], fn conn,
                                                                                          _auth,
                                                                                          _resource ->
  ...
  def update_folder(conn, folder_id) do
    content(conn, {:folder, folder_id}, [write: true], fn conn, _auth, _resource ->
      safely(
        conn,
  ...
  def delete_folder(conn, folder_id) do
    content(conn, {:folder, folder_id}, [user_only: true, write: true], fn conn,
                                                                           auth,
                                                                           _resource ->
      safely(
        conn,
  ...
  def create_note(conn, vault_id) do
    content(conn, {:vault, vault_id}, [write: true, error: "Could not create note"], fn conn,
                                                                                        auth,
                                                                                        _resource ->
  ...
  def get_note(conn, note_id) do
    content(conn, {:note, note_id}, fn conn, auth, note ->
      JSON.send(conn, 200, %{note: Privacy.redact_note(note, agent?(auth))})
    end)
  end

  def update_note(conn, note_id) do
    content(conn, {:note, note_id}, [write: true, error: "Could not update note"], fn conn,
                                                                                      auth,
                                                                                      existing ->
  ...
  def rename_note(conn, note_id) do
    content(conn, {:note, note_id}, [write: true, error: "Could not rename note"], fn conn,
                                                                                      auth,
                                                                                      _existing ->
  ...
  def delete_note(conn, note_id) do
    content(conn, {:note, note_id}, [write: true, error: "Could not delete note"], fn conn,
                                                                                      _auth,
                                                                                      existing ->
  ...
  def move_note(conn, note_id) do
    content(conn, {:note, note_id}, [write: true, error: "Could not move note"], fn conn,
                                                                                    auth,
                                                                                    _existing ->
      folder_id =
  ...
  def backlinks(conn, note_id) do
    content(conn, {:note, note_id}, fn conn, auth, _note ->
      conn = fetch_query_params(conn)
  ...
  def add_tag(conn, note_id) do
    content(
      conn,
      {:note, note_id},
      [write: true, viewer: "Viewer role cannot edit tags"],
      fn conn, auth, note ->
  ...
  def versions(conn, note_id) do
    content(conn, {:note, note_id}, [user_only: true], fn conn, _auth, _note ->
      JSON.send(conn, 200, %{versions: Versions.list(note_id)})
    end)
  end

  def diff(conn, note_id) do
    content(conn, {:note, note_id}, [user_only: true], fn conn, _auth, note ->
      conn = fetch_query_params(conn)
  ...
  def graph(conn, vault_id) do
    content(conn, {:vault, vault_id}, [user_only: true], fn conn, _auth, _resource ->
  ...
  def preview_html(conn, note_id, asset_id) do
    content(conn, {:note, note_id}, [user_only: true, missing: "Not found"], fn conn,
                                                                                _auth,
                                                                                _note ->
  ...
  defp serve_private_asset(conn, note_id, asset_id) do
    content(conn, {:note, note_id}, [missing: "Not found"], fn conn, _auth, _note ->
      case Assets.resolve_path(note_id, asset_id) do
        nil ->
          JSON.send(conn, 404, %{error: "Not found"})
  ...
  defp mutate_note_toggle(conn, note_id, fallback, operation) do
    content(conn, {:note, note_id}, [write: true, error: fallback], fn conn, auth, _existing ->
