  ...
  def context(channel_id, registration_id, trigger_id, worker? \\ false) do
    ...
    case registration(channel_id, registration_id) do
      [owner, 1, 1] when not worker? ->
        ...
        guidance =
          if allowed do
            "At an enabled idea checkpoint, offer one new bounded work suggestion after answering active user requests. #{Cascade.Missions.Context.delivery_guidance()} For non-delivery idea checkpoints, creativity is welcome: consider features, experiments and simplifications as well as repairs, with a plausible benefit. An observed defect or proven need is not required; label speculative benefits honestly. Separate factual premises from speculative benefits: verify concrete repository claims against current permitted source with proportionate checks and briefly name the supporting source, or state the premise itself as an unverified hypothesis without implying a defect exists. Before choosing an idea, compare the action and rationale with prior proposals, owner feedback and current work. When feedback undermines an idea, revise the approach rather than just rewording the question. Do not repackage declined work, repeat a pending question on an async receipt, or offer completed, active or already authorized work as a new proposal. Fulfill accepted obligations within scope; do not ask permission for already authorized work. Give one specific acceptance question for the new idea, never a generic add-on or request to grade you. Use only permitted project/chat/task context; proportionate read-only discovery is allowed, but no tools that implement proposed work until owner acceptance. Do not fabricate defects or optimize for spending tokens itself. Respect explicit Stop and declined topics. An ignored proposal is not acceptance and does not suppress future ideas at new checkpoints. When offering a new idea, prefix the reply with <!-- fizzer-next:#{trigger_id} --> followed by a blank line, then the answer and concise suggestion. For delivery without a proposal, use the ordinary completion summary/reply without a suggestion marker; do not withhold delivery to invent an idea. Respect [no-reply] after an already published delivery and explicit Stop. Never create a new checkpoint or background run merely to generate another suggestion."
          else
...
For owner feedback on a recorded proposal, prefix your ordinary reply with <!-- fizzer-next-feedback:PROPOSAL_MESSAGE_ID:#{trigger_id}:accepted|declined|redirected -->, choosing exactly one disposition only when the owner clearly expresses it. When offering a new idea alongside feedback, place its <!-- fizzer-next:#{trigger_id} --> marker immediately after the feedback marker, before your answer and suggestion. Ambiguity is not acceptance; leave prior feedback unchanged. Acceptance authorizes only that bounded proposal plus the owner's explicit constraints; do not broaden it. A decline suppresses that proposal; do not repeat it. A redirect replaces the topic only to the extent the owner requests."
        ...
        {:ok, message} =
          Cascade.Chat.Messages.create(
            ...
            %{
              ...
              body:
                "Next-step checkpoint (#{kind}). #{evidence} Apply the next-step guidance in the coordinator context. This checkpoint grants no authority to start work."
            },
            access: :agent
          )
