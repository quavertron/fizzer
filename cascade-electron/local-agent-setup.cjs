...
async function registerHermes(fetch, input) {
  ...
  async function api(method, route, body) {
    ...
    if (!response.ok) throw new Error(`Setup upstream HTTP ${response.status}`);
  ...
  else identity = (await api('PUT', root + '/vault-agents', { agentId: 'hermes',
...
async function startLocalAgentSetup({ fetch, directory = path.join(os.homedir(), '.cascade', 'agent-setup') }) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      reply(200, result);
    } catch { reply(400, { error: 'Setup refused or incomplete; inspect exact records before retrying' }); }
    finally { busy = false; }
  });
